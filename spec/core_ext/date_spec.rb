...
describe 'Date extensions' do
  before(:all) { GBWorkingDay::WorkingWeek.current = GBWorkingDay::WorkingWeek.new(5) }
  after(:all)  { GBWorkingDay::WorkingWeek.current = GBWorkingDay::WorkingWeek.new() }
  ...
  it 'should respond to work_date' do
    date = Date.today
    expect(date.work_date).to be_kind_of GBWorkingDay::Date
    expect(date.work_date).to eq date
  end

  it 'should respond to to_work_date' do
    date = Date.today
    expect(date.to_work_date).to be_kind_of GBWorkingDay::Date
    expect(date.to_work_date).to eq date
  end

  it 'should respond to to_work' do
    date = Date.today
    expect(date.to_work).to be_kind_of GBWorkingDay::Date
    expect(date.to_work).to eq date
  end

  it 'should properly subtract work dates' do
    expect(Date.today - Date.tomorrow.to_work_date).to be_a_kind_of GBWorkingDay::Duration
  end
